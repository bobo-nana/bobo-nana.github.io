...
class PostNode(Node):
    def render(self, site: Site) -> str:
        ...
        return f"""
            ...
            <div class="mb-4">
                <span>
                    <i class="bi bi-calendar-event" style="font-size: 1rem; color: cornflowerblue;"></i>
                    {date}
                </span>
...
class PNode(Node):
    def render(self, site: Site) -> str:
        return f"<p class=\"mb-3\">{self.attr.get('text')}</p>"
...
class TimelineItemNode(Node):
    def render(self, site: Site) -> str:
        ...
        if date:
            stamp = f"""<i class="bi bi-calendar-event" style="font-size: 1rem; color: dimgrey;"></i> {date}"""

            if time:
                stamp += f" {time}"
        elif time:
            stamp = f"""<i class="bi bi-clock" style="font-size: 1rem; color: dimgrey;"></i> {time}"""
        else:
            stamp = ""

        if pict:
            pict = f"""<img src="{pict}" class="img-fluid w-100 rounded pb-4" alt="{text}">"""
        ...
        return f"""
            <li class="timeline-item mb-5">
                {icon}
                {name}
                <p class="text-muted pb-2 fw-bold">
                    {stamp}
                </p>
                {pict}
                {text}
